...
void _gap_srch2(rb_gap_tree_t *T, rb_gap_node_t *X, rb_time_t c, rb_gap_t *gap, size_t *comp_steps)
{
     if (X != T->nil)
     {
...
rb_gap_t gap_srch2(rb_gap_tree_t *gaps, rb_time_t c, size_t *comp_steps)
{
     rb_gap_t gap = { -1, 0 };
...
rb_time_t gap_enum2(task_t *tasks, size_t n, size_t j, int windows, size_t *comp_steps, size_t *gap_ct)
{
     ...
     while (L < U)
     {
	  ...
	  {
	       (*comp_steps)++;
	       rb_gap_tree_t *gaps2 = gap_xfrm2((rb_gap_t){ 0, L }, gaps, tasks, i, comp_steps);
	       if (!gaps2)
	       {
		    *gap_ct = rb_size(gaps);
		    memset(buf, 0, 8192);
	  ...
	  rb_gap_t gap = gap_srch2(gaps, tasks[j].c, comp_steps);
	  rb_time_t rt_j = -1;
